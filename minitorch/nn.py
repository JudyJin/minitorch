...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    """  
    # COPY FROM ASSIGN2_2
    return max(input, dim) + (input - max(input, dim)).exp().sum(dim).log()
...
def one_hot(input: Tensor, num_classes: int) -> Tensor:
    ...
    """
    # COPY FROM ASSIGN2_2
    diagonal = np.eye(num_classes)
    return tensor_from_numpy(diagonal[input.to_numpy().astype(int)], backend=input.backend)
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    log_softmax = logsoftmax(logits, 1)
    result = -(target_one_hot * log_softmax).sum(1)
