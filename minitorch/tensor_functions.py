...
class PowerScalar(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # COPY FROM ASSIGN2_1
        grad_a = grad_output * scalar * a.f.pow_scalar_zip(a, scalar - 1)
        return (grad_a, 0.0)
...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN3_1 
        soft_inp = ctx.saved_values[0]
        return (out_grad.f.attn_softmax_bw(out_grad, soft_inp),
        0.0)
      #   END ASSIGN3_1
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
      #   BEGIN ASSIGN3_2 
        res, var, mean = inp.f.layernorm_fw(inp, gamma, beta)
...
class FlashAttn(Function):
    @staticmethod
    def forward(ctx:Context, q: Tensor, k: Tensor, v:Tensor, mask:Tensor, is_causal: Tensor) ->Tensor:
        o, l, m = q.f.flash_attn_fw(q,k,v,mask,is_causal)
        ctx.save_for_backward(q, k, v, o, l, m, mask, is_causal)
        return o
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
