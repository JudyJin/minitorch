    ...
    def self_attention(self, q, kT, v):
        ...
        if self.use_flash:
            ...
            if self.causal:
                ...
                mask = tensor_from_numpy(np.zeros((batch_size, self.n_head, queries_len, queries_len), dtype=datatype), backend=self.backend)
...
class FeedForward(Module):
    ...
    def forward(self, x):
        ...
        # COPY FROM ASSIGN2_4
        x = self.linear_in(x.view(batch_size * seq_len, n_embd))
        x = GELU(x)
        x = self.dropout(x)
        x = self.linear_out(x)
        x = x.view(batch_size, seq_len, n_embd)
...
class TransformerLayer(Module):
    ...
    def forward(self, x):
        ...
        if not self.use_fused_kernel:
            # COPY FROM ASSIGN2_4
            x_l = self.ln_1(x.view(batch_size * seq_len, x_dim))
            x_l = self.attention(x_l.view(batch_size, seq_len, x_dim))
            x = x + x_l
            x_2 = self.ln_2(x.view(batch_size * seq_len, x_dim))
            x_2 = self.ff(x_2.view(batch_size, seq_len, x_dim))
            x = x + x_2
            x = x.view(batch_size, seq_len, x_dim)
        
        else:
            # BEGIN ASSIGN3_3
            x_l = x.view(batch_size * seq_len, x_dim).layernorm(self.ln_1.weights.value, self.ln_1.bias.value)
            x_l = self.attention(x_l.view(batch_size, seq_len, x_dim))
            x = x + x_l
            x_2 = x.view(batch_size * seq_len, x_dim).layernorm(self.ln_2.weights.value, self.ln_2.bias.value)
            x_2 = self.ff(x_2.view(batch_size, seq_len, x_dim))
            x = x + x_2
            x = x.view(batch_size, seq_len, x_dim)
            # END ASSIGN3_3
...
class DecoderLM(Module):
    ...
    def forward(self, idx):
        ...
        if not self.use_fused_kernel:
            ...
            position_ids = tensor_from_numpy(np.arange(seq_len), backend=self.backend).view(1, seq_len)
            position_embeddings = self.position_embeddings(position_ids)
            assert position_embeddings.shape == (1, seq_len, self.n_embd)
            # Pass through each transformer Layer
            x = token_embeddings + position_embeddings
            x = self.dropout(x)
            assert x.shape == (batch_size, seq_len, self.n_embd)
            x = self.t_layer_1(x)
            x = self.t_layer_2(x)
            x = self.t_layer_3(x)
            x = self.t_layer_4(x)
            ...
            # Get correct shape
            x = self.lm_head(x.view(batch_size * seq_len, self.n_embd))
