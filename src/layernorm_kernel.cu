...
__global__ void ker_layer_norm(T *ln_res, T *vars, T *means, const T *inp,
  ...
  // Step 2
  float mean_dim = float(hidden_size) * 4;
  float reduce_val[2] = {l_sum, l_sum2};
  ...
  for (uint idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
    float4 val = inp_f4[idx];
    val.x = (val.x - s_mean) * s_var * scale_f4[idx].x + bias_f4[idx].x;
    val.y = (val.y - s_mean) * s_var * scale_f4[idx].y + bias_f4[idx].y;
    val.z = (val.z - s_mean) * s_var * scale_f4[idx].z + bias_f4[idx].z;
    val.w = (val.w - s_mean) * s_var * scale_f4[idx].w + bias_f4[idx].w;
    output_f4[idx] = val;
  }
...
__global__ void ker_ln_bw_dgamma_dbetta(T *gamma_grad, T *betta_grad,
  ...
  float betta_val = 0;
  float gamma_val = 0;
  float out_grad_val, inp_val;
  if (idx< width){
    for (int i = threadIdx.y; i < rows; i += TILE_DIM) {
      out_grad_val = out_grad[i * width + idx];
      inp_val = inp[i * width + idx];
      float xhat_val = (inp_val - means[i]) * rsqrtf(vars[i]+LN_EPSILON);
      betta_val += out_grad_val;
      gamma_val += xhat_val * out_grad_val;
    }
  }

  // Step 2
  betta_buffer[threadIdx.x][threadIdx.y] = betta_val;
  gamma_buffer[threadIdx.x][threadIdx.y] = gamma_val;
  __syncthreads();
  // Step 3
...
__global__ void ker_ln_bw_dinp(T *inp_grad, const T *out_grad, const T *inp,
  ...
  // Step 2
    ...
    vars_f = vars[blockIdx.x]+LN_EPSILON;
