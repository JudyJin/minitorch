...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    // int out_pos = batch * out_strides[0] + row * out_strides[1] + col;
    // 3. Iterate over tiles of the two input matrices, read the data into shared memory

    // printf("a_shape[1]: %d, a_shape[2]: %d\n", a_shape[1], a_shape[2]);
    // printf("b_shape[1]: %d, b_shape[2]: %d\n", b_shape[1], b_shape[2]);
    // printf("tlie: %d\n", TILE);
    float sum = 0;
    for (int i = 0; i < a_shape[2]/TILE+1; i++) {
      a_index[0] = batch;
      a_index[1] = row;
      a_index[2] = i*TILE + threadIdx.y;
      b_index[0] = batch;
      b_index[1] = i*TILE + threadIdx.x;
      b_index[2] = col;

      broadcast_index(a_index, a_shape, a_shape, a_index, 3, 3);
      broadcast_index(b_index, b_shape, b_shape, b_index, 3, 3);
      ...
      if (b_index[1] < b_shape[1] && b_index[2] < b_shape[2]) {
      ...
      // a_shared[threadIdx.y][threadIdx.x] = a_storage[batch * a_batch_stride + row * a_strides[1] + (i*TILE + threadIdx.x)];
      // b_shared[threadIdx.x][threadIdx.y] = b_storage[batch * b_batch_stride + (i*TILE + threadIdx.y) * b_strides[1] + col];
      ...
      for (int j = 0; j < TILE; j++) {
        if (i + j < a_shape[2]) {
          sum += a_shared[threadIdx.x][j] * b_shared[j][threadIdx.y];
        }
      }
...
__global__ void mapKernel(
   ...
   */
    ...
    // printf("pos:%d\n:",index_to_position(in_index, in_strides, shape_size));
    // printf("in_pos: %d\n", in_pos);
    ...
    // print the entire out array
    // printf("in_pos: %d, in_storage[in_pos]: %f\n", in_pos, in_storage[in_pos]);
    // printf("out_pos: %d, out[out_pos]: %f\n", out_pos, out[out_pos]);
