...
void launch_attn_softmax_bw(float *out_grad,
  ...
  cudaMalloc((void **)&d_out_grad, rows * softmax_len * sizeof(float));
  cudaMalloc((void **)&d_soft_inp, rows * softmax_len * sizeof(float));
  cudaMemcpy(d_out_grad, out_grad, rows * softmax_len * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft_inp, soft_inp, rows * softmax_len * sizeof(float), cudaMemcpyHostToDevice);
  // Launch kernel
  ...
  // Copy back to the host
  cudaMemcpy(out_grad, d_out_grad, rows * softmax_len * sizeof(float), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
  // Free memory on device
  cudaFree(d_out_grad);
  cudaFree(d_soft_inp);
