...
def collate_batch(
    ...
    # model_max_length = model_max_length + 1
    for example in examples:
        ...
        token_id = token_ids_src + token_ids_tgt + [pad_token_id] * (model_max_length - len(token_ids_src) - len(token_ids_tgt))
        mask = [0] * len(token_ids_src) + [1] * len(token_ids_tgt) + [0] * (model_max_length - len(token_ids_src) - len(token_ids_tgt))

        token_ids.append(token_id[:model_max_length])
        tgt_token_mask.append(mask[:model_max_length])

    # COPY FROM ASSIGN2_5
    input_ids = minitorch.tensor_from_numpy(np.array(token_ids)[:,:-1], backend = backend)
    labels = minitorch.tensor_from_numpy(np.array(token_ids)[:,1:], backend = backend)
    label_token_weights = minitorch.tensor_from_numpy(np.array(tgt_token_mask)[:,1:], backend = backend)
    return {
        'input_ids': input_ids,
        'labels': labels,
...
def loss_fn(batch, model):
    ...
    labels = batch['labels'].view(batch_size * seq_len)
    label_token_weights = batch['label_token_weights'].view(batch_size * seq_len)

    loss = minitorch.nn.softmax_loss(logits, labels)
    loss = loss * label_token_weights
    return loss.sum() / label_token_weights.sum()
...
def main(dataset_name='bbaaaa/iwslt14-de-en-preprocess',
    ...
    with open('use fused kernel '+str(use_fused_kernel)+' time.txt', 'w') as f:
        f.write('Training time: '+str(end_time - start_time)+' seconds\n')
